...
-(WMZBannerParam *)ht_getWMZHomeBannerParam {
   ...
   .wLineSpacingSet(self.LineSpacing)
    ...
    .wEventClickSet(^(id anyID, NSInteger index) {
        ...
        if ([weakSelf.delegate respondsToSelector:@selector(ht_homeBannerDidSelect:)]) {
            return [weakSelf.delegate ht_homeBannerDidSelect:index];
        }
    ...
    .wEventScrollEndSet(^(id anyID, NSInteger index,BOOL isCenter,UICollectionViewCell *cell) {
        ...
        if ([weakSelf.delegate respondsToSelector:@selector(ht_homeBannerScrollEnd:)]) {
            return [weakSelf.delegate ht_homeBannerScrollEnd:index];
        }
    })
   ;
